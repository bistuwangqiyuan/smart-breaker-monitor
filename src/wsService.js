...
class WebSocketService {
  ...
  init(port = 8080) {
    try {
      ...
      this.logger.info(`WebSocket服务器启动在端口 ${port}`);
      ...
      this.logger.error(`WebSocket服务器初始化失败: ${error.message}`);
      return false;
    }
  }
  ...
  handleConnection(ws) {
    // 添加到客户端集合
    this.clients.add(ws);
    this.logger.info(`新的WebSocket客户端连接，当前连接数: ${this.clients.size}`);
    ...
    ws.on('message', (message) => {
      try {
        ...
        this.logger.error(`处理客户端消息失败: ${error.message}`);
      }
    });
    ...
    ws.on('close', () => {
      ...
      this.logger.info(`WebSocket客户端断开连接，当前连接数: ${this.clients.size}`);
    });
  }
